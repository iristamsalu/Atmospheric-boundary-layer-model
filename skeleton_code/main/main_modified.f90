...
!added 21.01.25 by me, temporary                             
real(dp), parameter :: K_m = 5.0      ! [m^2/s]                             
...
do while (time <= time_end)
  ...
  call surface_values(theta(1), time+dt)  ! theta = temperature at the 

  ! Update 
  do hh_index = 2,  len(hh)-1
    print *, 'Entering loop for hh_index = ', hh_index
    ! Altitude differentials
    ! dhh_nxt = hh(hh_index+1) - hh(hh_index)
    ! dhh_prvs = hh(hh_index) - hh(hh_index-1)
    ! dhh_avg = (hh(hh_index+1) - hh(hh_index-1)) / 2.0

    ! du_dt = fcor * (vwind(hh_index) - vg) + &
    !         (K_m * (uwind(hh_index+1) - uwind(hh_index)) / (hh(hh_index+1) - hh(hh_index)) - &
    !         K_m * (uwind(hh_index) - uwind(hh_index)) / hh(hh_index) - hh(hh_index-1)) / &
    !         (hh(hh_index+1) - hh(hh_index-1)) / 2.0

    uwind(hh_index) = uwind(hh_index) + dt * &
                      fcor * (vwind(hh_index) - vg) + &
                      (K_m * (uwind(hh_index+1) - uwind(hh_index)) / (hh(hh_index+1) - hh(hh_index)) - &
                      K_m * (uwind(hh_index) - uwind(hh_index-1)) / hh(hh_index) - hh(hh_index-1)) / &
                      (hh(hh_index+1) - hh(hh_index-1)) / 2.0

    print *, 'Time: ', time
    print *, 'uwind before update at hh_index: ', hh_index, ' = ', uwind(hh_index)
    print *, 'Right-hand side of the update: ', dt * (fcor * (vwind(hh_index) - vg) + & 
                                                (K_m * (uwind(hh_index+1) - uwind(hh_index)) / (hh(hh_index+1) - hh(hh_index)) - &
                                                K_m * (uwind(hh_index) - uwind(hh_index-1)) / (hh(hh_index) - hh(hh_index-1))) / &
                                                ((hh(hh_index+1) - hh(hh_index-1)) / 2.0))
...
contains
!-----------------------------------------------------------------------------------------
! subroutine select_K_m()
!
! Selects one of the three models
!-----------------------------------------------------------------------------------------
!subroutine select_K_m(model_nr)
  ! integer :: model_nr

  ! if (model_nr == 1) then
  !   K_m = 5.0 ! m^2/s
  ! else if (model_nr == 2) then
  !   K_m = 10.0 ! m^2/s
  ! else
  !   K_m = 15.0 ! m^2/s
  ! end subroutine select_K_m
...
subroutine meteorology_init()
  ! Wind velocity
  ! Sets all elements in an array to 0.0d0 [m s-1]
  uwind         = 0.0d0 
  ! Sets u wind velocity of the last element to geostrophic wind velocity 10.0d0 [m s-1] 
  uwind(nz)     = ug 
  ! Calculates u velocity for the second element to the second-to-last element &
  ! by scaling the u velocity at the highest altitude by altitude ratio 
  uwind(2:nz-1) = uwind(nz) * hh(2:nz-1)/hh(nz) 

  ! Sets all elements in an array to 0.0d0 [m s-1]
  vwind = 0.0d0
  ! Sets the last element equal to geostrophic wind 0.0d0 [m s-1] 
  vwind(nz) = vg

  ! Potential temperature
  ! Sets all potential temperatures
  theta     = 273.15d0 + 25.0d0
  ! The last potential temperature in an array is different
  theta(nz) = 273.15d0 + 30.0d0

  ! Air temperature and pressure
  ! Set temperatures for every altitude
  temp = theta - (grav/Cp)*hh 
  ! Set pressures for every altitude with the barometric law function
