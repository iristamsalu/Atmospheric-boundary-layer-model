...
DO WHILE (time <= time_end)
  ...
  if ( use_emission .and. time >= time_start_emission ) then
    if ( mod( nint((time - time_start_emission)*1000.0d0), nint(dt_emis*1000.0d0) ) == 0 ) then
      ...
      call get_emissions(time, daynumber_start, latitude, temp(2), Rgas, F_veg_isoprene, F_veg_monoterpene)
...
subroutine open_files()
  ...
  open(16,file=trim(adjustl(output_dir))//'/Isoprene.dat',status='replace',action='write')
  open(17,file=trim(adjustl(output_dir))//'/Monoterpene.dat',status='replace',action='write')
...
subroutine write_files(time)
  ...
  write(16, outfmt_level     ) F_veg_isoprene
  write(17, outfmt_level     ) F_veg_monoterpene
...
! Get emissions
!-----------------------------------------------------------------------------------------
subroutine get_emissions(time, daynumber_start, latitude, temp, Rgas, F_veg_isoprene, F_veg_monoterpene)

real(dp), intent(in)                 :: time, latitude, temp, Rgas, daynumber_start
...
! calculate daynumber
daynumber = float(daynumber_start) + time / 24.0_dp
...
F_veg_monoterpene = D_m * eeta * gamma_isoprene * delta
! Make sure F_veg is in correct units

F_veg_isoprene = 1.0_dp
F_veg_monoterpene = 1.0_dp
