...
do while (time <= time_end)
  ...
  do hh_index = 2, size(hh) - 1
    ...
    vwind(hh_index) = vwind(hh_index) + dt * ( &
                     ...
                     (K_m * (vwind(hh_index+1) - vwind(hh_index)) / (hh(hh_index+1) - hh(hh_index)) - &
                     K_m * (vwind(hh_index) - vwind(hh_index-1)) / (hh(hh_index) - hh(hh_index-1))) / &
                     ((hh(hh_index+1) - hh(hh_index-1)) / 2.0))

    ! Update potential temperature
    theta(hh_index) = theta(hh_index) + dt * ( &
                     K_m * (theta(hh_index+1) - theta(hh_index)) / (hh(hh_index+1) - hh(hh_index)) - &
                     K_m * (theta(hh_index) - theta(hh_index-1)) / (hh(hh_index) - hh(hh_index-1))) / &
                     ((hh(hh_index+1) - hh(hh_index-1)) / 2.0)                 
  end do
